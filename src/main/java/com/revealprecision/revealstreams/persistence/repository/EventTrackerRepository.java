...
public interface EventTrackerRepository extends JpaRepository<EventTracker, UUID> {
  ...
  @Query(value = "SELECT CAST(lp.identifier as varchar) as locationIdentifier "
      ...
      + ",sum(COALESCE(CAST((et.observations->'untreated_travel'->>0) as int),0)) as totalUntreatedTravel \n"
      ...
      + "\t COALESCE(CAST((et.observations->'untreated_travel'->>0) as int),0) +     \n"
...
//      + "\t COALESCE(CAST((et.observations->'untreated_childrenu5'->>0) as int),0) +     \n"
  ...
  @Query(value = "SELECT hoh_aggregate.head_of_household              as householdHead\n"
      ...
      + "     , hoh_aggregate.totalUntreatedTravel           as totalUntreatedTravel\n"
      + "     , hoh_aggregate.totalUntreated                 as totalUntreated\n"
      + "     , hoh_aggregate.tablets                        as tablets\n"
      + "     , hoh.business_status                          as businessStatus "
      + "from (\n"
      ...
      + "              , sum(COALESCE(CAST((hh.observations -> 'untreated_travel' ->> 0) as int),\n"
      ...
      + "                    COALESCE(CAST((hh.observations -> 'untreated_travel' ->> 0) as int), 0) +\n"
      + "                    COALESCE(CAST((hh.observations -> 'untreated_sick' ->> 0) as int),\n"
      + "                             0)) as totalUntreated\n"
      + "              , sum(COALESCE(CAST((hh.observations -> 'tablets' ->> 0) as int),\n"
      + "                             0)) as tablets\n"
      + "         from (\n"
      + "                  SELECT case\n"
      ...
      + "           et.observations -> 'business_status' ->> 0 as business_status\n"
      + "    from event_tracker et\n"
      + "    WHERE et.event_type = 'mda_onchocerciasis_survey'\n"
